...
const LIGHT_PREFS = [
  { id: 'any', icon: 'ellipsis-horizontal', labelKey: 'lightAny' },
  { id: 'low', icon: 'moon-outline', labelKey: 'lightLow' },
  { id: 'medium', icon: 'partly-sunny-outline', labelKey: 'lightMedium' },
  { id: 'high', icon: 'sunny-outline', labelKey: 'lightHigh' },
];

// Temperature preference filter
const TEMP_PREFS = [
  { id: 'any', icon: 'ellipsis-horizontal', labelKey: 'tempAny' },
  { id: 'cool', icon: 'snow-outline', labelKey: 'tempCool' },
  { id: 'room', icon: 'home-outline', labelKey: 'tempRoom' },
  { id: 'warm', icon: 'flame-outline', labelKey: 'tempWarm' },
];
...
export default function TerrariumBuilderScreen() {
  ...
  // Load plants for first selection - filtered by terrarium type + light/temp prefs (OFFLINE)
  const loadPlantsForFirstSelection = useCallback(async () => {
    setLoading(true);
    try {
      const terrariumType = getTerrariumType();
      const result = await getPlants(undefined, terrariumType, undefined, false, undefined, container.lightPref, container.tempPref);
  ...
  }, [getTerrariumType, container.lightPref, container.tempPref]);
  ...
  const renderContainerStep = () => (
    <ScrollView style={styles.stepContainer} showsVerticalScrollIndicator={false}>
      ...
      {/* Light Preference (optional) */}
      <Text style={styles.sectionLabel}>{t('lightPreference') || 'Fényigény'}</Text>
      <View style={styles.optionRow}>
        {LIGHT_PREFS.map(lp => (
          <TouchableOpacity
            key={lp.id}
            style={[
              styles.optionButton,
              container.lightPref === lp.id && styles.optionButtonSelected,
            ]}
            onPress={() => setContainer(prev => ({ ...prev, lightPref: lp.id }))}
          >
            <Ionicons
              name={lp.icon as any}
              size={28}
              color={container.lightPref === lp.id ? '#fff' : '#388E3C'}
            />
            <Text style={[
              styles.optionLabel,
              container.lightPref === lp.id && styles.optionLabelSelected,
            ]}>
              {t(lp.labelKey as any)}
            </Text>
          </TouchableOpacity>
        ))}
      </View>

      {/* Temperature Preference (optional) */}
      <Text style={styles.sectionLabel}>{t('tempPreference') || 'Hőmérséklet'}</Text>
      <View style={styles.optionRow}>
        {TEMP_PREFS.map(tp => (
          <TouchableOpacity
            key={tp.id}
            style={[
              styles.optionButton,
              container.tempPref === tp.id && styles.optionButtonSelected,
            ]}
            onPress={() => setContainer(prev => ({ ...prev, tempPref: tp.id }))}
          >
            <Ionicons
              name={tp.icon as any}
              size={28}
              color={container.tempPref === tp.id ? '#fff' : '#388E3C'}
            />
            <Text style={[
              styles.optionLabel,
              container.tempPref === tp.id && styles.optionLabelSelected,
            ]}>
      ...
      <ScrollView style={styles.stepContainer} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            setContainer({ shape: null, opening: null, size: null, terrariumType: null, lightPref: 'any', tempPref: 'any' });
          }}
        >
