...
// Helper: look up image from bundled data by plant name
const getPlantImage = (name: string): string | undefined => {
  const plant = (plantsData as any[]).find((p: any) => p.name === name);
...
export default function MyTerrariumsScreen() {
  ...
  const exportPDF = async (terrarium: SavedTerrarium) => {
    ...
    // Enrich plants with images from bundled data
    const plantsWithImages = terrarium.plants.map(p => ({
      ...p,
      image_base64: p.image_base64 || getPlantImage(p.name),
    }));

    const plantCards = plantsWithImages.map((p, i) => {
      const imgTag = p.image_base64
    ...
    const plantProfilesHtml = plantsWithImages.map(p => plantProfileHtml(p)).join('');
          ...
          <View key={terr.id} style={styles.card}>
            ...
            <View style={styles.plantRow}>
              {terr.plants.slice(0, 6).map((p) => {
                const img = p.image_base64 || getPlantImage(p.name);
                return (
                  <TouchableOpacity
                    key={p.name}
                    style={styles.thumb}
